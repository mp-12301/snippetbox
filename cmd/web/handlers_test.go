...
func TestPing(t *testing.T) {
	...
	ts := newTestServer(t, app.routes(config{StaticDir: ""}))
	defer ts.Close()

	st, _, body := ts.get(t, "/ping")

	if st != http.StatusOK {
		t.Errorf("want %d; got %d", http.StatusOK, st)
	}
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Duplicate email", "Bob", "dupe@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("Email is already in use")},
...
func TestCreateSnippetForm(t *testing.T) {
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		if headers.Get("Location") != "/user/login" {
			t.Errorf("want location %s; got %s", "/user/login", headers.Get("Location"))
		}
	})
	...
	t.Run("Authenticated", func(t *testing.T) {
		...
		if code != http.StatusOK {
			t.Errorf("want code %d; got %d", http.StatusOK, code)
		}

		if !bytes.Contains(body, []byte("<form action='/snippet/create' method='POST'>")) {

			t.Errorf("want body %s to contain %q", body, "<form action='/snippet/create' method='POST'>")
		}
